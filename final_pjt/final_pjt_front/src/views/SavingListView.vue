  ...
  <div v-if="data" class="container-out">
    <div class="container-in">
      <div class="search-section">
        ...
        <!-- 조회 버튼 -->
        <button type="submit" class="btn" @click="search">조회</button>
      </div>
      ...
      <div class="result-section">
        ...
        <div class="table-responsive">
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="save in data" :key="save.id">
                ...
                <td>
                  <RouterLink
                    :to="{
                      ...
                      params: { fin_prdt_cd: save.fin_prdt_cd },
                    }"
                    class="link-line"
                    >{{ save.fin_prdt_nm }}</RouterLink
                  >
                </td>
                <td>{{ save["6"] }}</td>
                <td>{{ save["12"] }}</td>
...
// 검색 메서드
const search = () => {
  data.value = store.savingBaseList.filter((saving) => {
    return (
      (bank.value === "" || saving.kor_co_nm === bank.value) &&
      (period.value === "" || saving[period.value] !== undefined)
    );
  });
};
...
onMounted(() => {
  store.getSavingBaseList();
  store.getSavingOptionList();
  data.value = store.savingBaseList;
});
</script>
