<template>
  <div v-if="save" class="container mt-5">
    <div class="card">
      <div class="card-header text-center">
        <h1>{{ save.fin_prdt_nm }} 상세페이지</h1>
      </div>
      <div class="card-body d-flex flex-column">
        <ul class="list-group list-group-flush flex-grow-1">
          <li class="list-group-item flex-grow-1 d-flex align-items-center">
            <strong>공시 제출일:</strong>
            <span class="ms-2">{{ save.dcls_month }}</span>
          </li>
          <li class="list-group-item flex-grow-1 d-flex align-items-center">
            <strong>금융회사명:</strong>
            <span class="ms-2">{{ save.kor_co_nm }}</span>
          </li>
          <li class="list-group-item flex-grow-1 d-flex align-items-center">
            <strong>상품명:</strong>
            <span class="ms-2">{{ save.fin_prdt_nm }}</span>
          </li>
          <li class="list-group-item flex-grow-1 d-flex align-items-center">
            <strong>가입제한:</strong>
            <span class="ms-2">{{ save.join_deny }}</span>
          </li>
          <li class="list-group-item flex-grow-1 d-flex align-items-center">
            <strong>가입 방법:</strong>
            <span class="ms-2">{{ save.join_way }}</span>
          </li>
          <li class="list-group-item flex-grow-1 d-flex align-items-center">
            <strong>우대조건:</strong>
            <span class="ms-2">{{ save.mtrt_int }}</span>
          </li>
        </ul>
      </div>
...
const getPortFolio = function () {
  axios({
    ...
    .then((response) => {
      if (
        response.data.find(
          (item) => item.fin_prdt_cd === save.value.fin_prdt_cd
        )
      ) {
...
onMounted(() => {
  save.value = fstore.savingBaseList.find(
